...
public:
    tp_reassembler(uint32_t _max_message_size, boost::asio::io_context& _io);
    /**
     ...
     */
    std::pair<bool, message_buffer_t> process_tp_message(const byte_t* const _data, uint32_t _data_size,
...
private:
    ...
    std::mutex mutex_;
    std::map<boost::asio::ip::address, std::map<uint16_t, std::map<uint64_t, std::pair<session_t, tp_message>>>> tp_messages_;
};
