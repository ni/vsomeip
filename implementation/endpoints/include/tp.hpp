...
typedef uint32_t tp_header_t;
typedef uint8_t tp_message_type_t;
typedef std::vector<message_buffer_ptr_t> tp_split_messages_t;

const uint8_t TP_FLAG = 0x20;
...
public:
    ...
    static tp_split_messages_t tp_split_message(const uint8_t* const _data, uint32_t _size, uint16_t _max_segment_length);

    static const uint16_t tp_max_segment_length_ = 1392;
};
