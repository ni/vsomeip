...
private:
    ...
    void receive_cbk(boost::system::error_code const& _error, size_t _bytes, const message_buffer_ptr_t& _recv_buffer,
                     size_t _recv_buffer_size);

    void connect();
    void receive();
    void receive(message_buffer_ptr_t _recv_buffer, size_t _recv_buffer_size, size_t _missing_capacity);
    void calculate_shrink_count(const message_buffer_ptr_t& _recv_buffer, size_t _recv_buffer_size);
    std::string get_address_port_remote() const;
    std::string get_address_port_local() const;
    void handle_recv_buffer_exception(const std::exception& _e, const message_buffer_ptr_t& _recv_buffer, size_t _recv_buffer_size);
