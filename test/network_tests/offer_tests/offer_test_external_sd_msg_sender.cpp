...
TEST(someip_offer_test, send_offer_service_sd_message) {
    try {
        ...
        for (bool keep_receiving = true; keep_receiving;) {
            size_t bytes_transferred = udp_socket.receive(boost::asio::buffer(receive_buffer, receive_buffer.capacity()), 0, ec);
        ...
        uint8_t its_offer_service_message[] = {0xff, 0xff, 0x81, 0x00, 0x00, 0x00, 0x00, 0x3c, 0x00, 0x00, 0x00, 0x01, 0x01,
